...
func (a App) generateTree() {
	a.logger.Infof("Try to create app folder %s", a.data.AppName)

	if err := os.Mkdir(a.data.AppName, 0755); err != nil {
		if !errors.Is(err, os.ErrExist) {
			a.logger.Errorf("Error creating folder %s", err)
		...
		a.logger.Debugf("Folder %s already exists", a.data.AppName)
	}

	a.readTemplates("templates")
}
